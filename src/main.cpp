...
const string strMessageMagic = "uTipcoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
   ...
   if (pindexBest->nHeight <= 1)
      {
        int64_t nSubsidy = 50000000 * COIN;
        return nSubsidy + nFees;
      }
      
    else if (pindexBest->nHeight <= 960)
      {
        int64_t nSubsidy = 1.13371337 * COIN;
        return nSubsidy + nFees;
      }
      
    else if (pindexBest->nHeight <= 999999)
      {
        int64_t nSubsidy = 13.13371337 * COIN;
        return nSubsidy + nFees;
      }      
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "uTipcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
